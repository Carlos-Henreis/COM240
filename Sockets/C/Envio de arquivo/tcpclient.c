...
int main() {
	...
	while(1){//loop para as mensagens
		...
		if (!arquivo) {
			...
			send(sock, nome_arq,TAMNOME, 0);//A mensagem é enviada
			//Até aqui funciona
			while (!feof(arquivo)) {
		       	fread(buffer, TAMBUFFER, 1, arquivo);
				send(sock,buffer,TAMBUFFER, 0);//A mensagem é enviada
		...
		printf("Arquivo foi recebido e persistido no servidor\n");
